...
function buildLocalURL(localPath) {
    return `file:///${path.resolve(__dirname, DEFAULT_START_PAGE)}`;
}

function parseCommandLineArguments(args) {
    program
            .version(version, '-v, --version')
            .option('-s, --start-page <page>', 'sets the starting page from which the sandbox to start')
            .option('-c, --chromium-path <path>', 'sets the chromium executable path for the sandbox to launch')
            .option('-d, --show-devtools', 'specifies whether or not to show devtools when launched')
            .option('--no-show-devtools', 'sets to not show devtools when launched')
            .option('--delegate-console', 'sets whether or not to delegate the browser\'s console to the sandbox console')
            .option('--no-delegate-console', 'sets to use the browser\'s default console')
            .option('-S, --slow-motion [time]', 'runs the sandbox in slow motion with the given time as delay (optional)')
            ...
            .option('-v, --viewport <width>:<height>', 'sets the viewport size of the sandbox')
            .option('-m, --mobile', 'runs sandbox in mobile mode')
            .option('-L, --landscape', 'runs sandbox in landscape mode')
            .option('-t, --timeout [time]', 'closes sandbox after the given amount of time (in ms) (default: 5 mins)')
            .option('-l, --generate-log-file [filename]', 'sets whether or not to generate a log file for console output')
            .option('--log-output-path <path>', 'sets the output path of the log file (default: logs)')
            .option('--default-output-path <path>', 'sets the default output path of sandbox (default: sandbox)')
...
function computeDefaultValues() {
    ...
    if (program.generateLogFile) {
        if (typeof program.generateLogFile == 'string' || program.generateLogFile instanceof String) {
            computed.logFilename = sanitizeFilename(program.generateLogFile);
        } else {
            computed.logFilename = DEFAULT_LOG_FILENAME;
        }

        computed.logFilename = `${computed.logFilename}-${dateformat(new Date(), 'yyyy-mm-dd')}.log`;
    }
...
function launchBrowser(computedValues) {
    if (computedValues.generateLogFile) {
        const resolvedLogOutputPath = path.resolve(__dirname, computedValues.logOutputPath);
        ...
        .pipe(
            fs.createWriteStream(
                path.join(
                    resolvedLogOutputPath,
                    computedValues.logFilename
                ), {
                    flags: 'a',
                    autoClose: true
                }
    ...
    puppeteer
            ...
            .then((page) => {
                ...
                page.goto(computedValues.startPage)
                    .then((response) => {
